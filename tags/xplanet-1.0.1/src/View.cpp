...
void
View::XYZToPixel(const double rotate_angle, 
		 const double X, const double Y, const double Z, 
		 double &newX, double &newY, double &newZ) const
{
    double X0, Y0, Z0;
    RotateToViewCoordinates(X, Y, Z, X0, Y0, Z0);

    const double cos_rot = cos(rotate_angle);
    const double sin_rot = sin(rotate_angle);

    newX =  cos_rot * X0 + sin_rot * Y0;
    newY = -sin_rot * X0 + cos_rot * Y0;
    newZ = Z0;
    
    newX *= -(distToPlane/(newZ*distPerPixel));
    newY *= -(distToPlane/(newZ*distPerPixel));
}

// Given pixel position (X, Y), return (X, Y, Z) in view coordinates
void
View::PixelToViewCoordinates(const double rotate_angle, 
			     ...
			     double &newX, double &newY, double &newZ)
{
    ...
    newX =  cos_rot * X + sin_rot * Y;
    newY = -sin_rot * X + cos_rot * Y;
